...
export function BandEditorForm({ band, onSave, onDelete, onCancel }: BandEditorFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/20 dark:bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
      <div 
        className="relative w-full max-w-xl bg-white dark:bg-[#0D0D0D] border-2 rounded-xl overflow-hidden shadow-2xl overflow-y-auto max-h-[90vh]"
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input 
                  type="text" 
                  maxLength={2}
                  className="w-full bg-gray-50 dark:bg-black/50 border border-gray-300 dark:border-gray-800 rounded px-3 py-2 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
            ...
            <div className="space-y-1">
              ...
              <select 
                className="w-full bg-gray-50 dark:bg-black/50 border border-gray-300 dark:border-gray-800 rounded px-3 py-2 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors capitalize"
                value={formData.category}
                onChange={e => setFormData({ ...formData, category: e.target.value as any })}
              >
                {Object.entries(categories).map(([key, cat]) => (
                  <option key={key} value={key} className="bg-white dark:bg-[#0D0D0D] text-gray-900 dark:text-white">{cat.name}</option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-1">
              ...
              <textarea 
                className="w-full bg-gray-50 dark:bg-black/50 border border-gray-300 dark:border-gray-800 rounded px-3 py-2 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors min-h-[80px]"
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input 
                  type="text" 
                  className="flex-1 bg-gray-50 dark:bg-black/50 border border-gray-300 dark:border-gray-800 rounded px-3 py-2 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
                  value={albumInput}
                ...
                <button 
                  type="button"
                  onClick={addAlbum}
                  className="px-4 py-2 border rounded border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors text-gray-900 dark:text-white"
                >
                  Add
                </button>
              </div>
              <div className="flex flex-wrap gap-2 pt-1">
                {formData.topAlbums?.map((album, i) => (
                  <span key={i} className="flex items-center gap-1 px-2 py-1 rounded bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-xs text-gray-700 dark:text-gray-300">
                    {album.title}
